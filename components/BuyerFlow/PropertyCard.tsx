    ...
    >
      <div className="block w-full relative">
        ...
        <div onClick={handleFavoriteClick} className="absolute top-2 right-2 bg-white/80 backdrop-blur-sm p-2 rounded-full cursor-pointer hover:bg-white z-10">
             <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 transition-colors duration-300 ${isFavorited ? 'text-red-500 fill-current' : 'text-neutral-500 hover:text-red-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="p-3 flex flex-col flex-grow">
        <button onClick={handleCardClick} className="text-left flex-grow">
            <div className="flex items-center text-neutral-600">
                <MapPinIcon className="w-3.5 h-3.5 mr-1.5 text-neutral-400 flex-shrink-0" />
                <span className="truncate text-xs">{property.address}, {property.city}</span>
            </div>
            ...
            <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-neutral-700">
                <div className="flex items-center gap-1" title={`${property.beds} bedrooms`}>
                    <BedIcon className="w-3.5 h-3.5 text-neutral-500" />
                    <span className="font-semibold text-xs">{property.beds}</span>
                </div>
                <div className="flex items-center gap-1" title={`${property.baths} bathrooms`}>
                    <BathIcon className="w-3.5 h-3.5 text-neutral-500" />
                    <span className="font-semibold text-xs">{property.baths}</span>
                </div>
                <div className="flex items-center gap-1" title={`${property.livingRooms} living rooms`}>
                    <LivingRoomIcon className="w-3.5 h-3.5 text-neutral-500" />
                    <span className="font-semibold text-xs">{property.livingRooms}</span>
                </div>
        ...
        <div className="mt-3 pt-3 border-t border-neutral-100 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-2">
            {showCompareButton && (
                <button
                    ...
                    className={`flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all shadow-sm hover:shadow-md w-full sm:w-auto ${
                        isInComparison
             ...
             <button
                ...
                className="bg-primary text-white px-3 py-1.5 rounded-full text-xs font-semibold hover:bg-primary-dark transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-2 w-full sm:w-auto"
            >
                {property.seller.avatarUrl ? (
                    <img src={property.seller.avatarUrl} alt={property.seller.name} className="w-5 h-5 rounded-full object-cover border border-white/50" />
                ) : (
